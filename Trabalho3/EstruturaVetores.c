...
{
    posicao--;
    if (posicao < 0 || posicao > 9) // se posição é um valor válido {entre 1 e 10}
        return POSICAO_INVALIDA;
    if (vetorPrincipal[posicao].vetorSecundario != NULL) // na posicao pode já existir estrutura auxiliar
        return JA_TEM_ESTRUTURA_AUXILIAR;
    if (tamanho < 1) // o tamanho nao pode ser menor que 1
        return TAMANHO_INVALIDO;
    vetorPrincipal[posicao].vetorSecundario = malloc(tamanho * sizeof(int));
    if (vetorPrincipal[posicao].vetorSecundario == NULL)
        return SEM_ESPACO_DE_MEMORIA; // o tamanho ser muito grande
    //

    // deu tudo certo, crie
    vetorPrincipal[posicao].size = tamanho;
    vetorPrincipal[posicao].topo = 0;
    if (isFirst)
    {
...
{
    long size = getFileSize("arquivo.txt");

    posicao--;
    if (posicao > 9 || posicao < 0)
        return POSICAO_INVALIDA;
    if (vetorPrincipal[posicao].vetorSecundario == NULL && size == 0)
        return SEM_ESTRUTURA_AUXILIAR;
    //
    // printf("passou da validação!\n");
    ...
    while (fgets(string, 1000, arquivo))
    {
        strcspn(string, "\n");
        strcspn(string, "\r");

        index = atoi(strtok(string, separador));
        // printf("index: %d\n\n", index);
        if (posicao == index)
        {
            // printf("Alocando\n\n");
